...
struct akai_ddtake_s{
	...
	u_char dummy1; /* XXX 0x01 */
	u_char stype; /* sample type (stereo=0x01, mono=0x00) */
...
#define DISK_TYPE_HD	3 /* S1000/S3000 harddisk */
	...
	u_int totsize; /* total size in blocks */
};
...
extern int open_disk(char *name,int readonly);

extern u_int akai_disksize(struct disk_s *dp);
...
extern void akai_list_volparam(struct akai_volparam_s *parp);
...
extern int format_harddisk(struct disk_s *dp,u_int bsize,u_int totb,int s3000flag,int cdromflag);
extern int format_harddisk9(struct disk_s *dp,u_int totb);
extern int format_floppy(struct disk_s *dp,int lodensflag,int s3000flag,int s900flag);
